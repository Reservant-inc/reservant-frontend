...
const VisitDetails: React.FC = () => {
  ...
  if (!visitDetails) {
    return (
      <div className="text-center">
        <p className="text-grey-5 dark:text-white">
          {t('customer-service.visit-details.no-visit-details')}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="flex h-full w-full gap-2 bg-grey-1 dark:bg-grey-5 dark:text-white">
      <div className="flex flex-col gap-2 h-full w-1/2">
        ...
        <div className="flex flex-col bg-white dark:bg-black rounded-lg p-4 shadow-md h-1/2">
          <h1 className="text-lg font-mont-bd">
            {t('customer-service.visit-details.visit-details')}
          </h1>
          <div className="flex flex-col gap-2 mt-4 px-4">
            <p>
              <span className="font-mont-bd">
                {t('customer-service.visit-details.date')}:
              </span>{' '}
              {new Date(visitDetails.date).toLocaleDateString()}
            </p>
            <p>
              <span className="font-mont-bd">
                {t('customer-service.visit-details.time')}:
              </span>{' '}
            ...
            <p>
              <span className="font-mont-bd">
                {t('customer-service.visit-details.guests')}:
              </span>{' '}
              {visitDetails.numberOfGuests}
            </p>
            <p>
              <span className="font-mont-bd">
                {t('customer-service.visit-details.takeaway')}:
              </span>{' '}
              {visitDetails.takeaway
                ? t('customer-service.visit-details.yes')
                : t('customer-service.visit-details.no')}
            </p>
            <p>
              <span className="font-mont-bd">
                {t('customer-service.visit-details.table-id')}:
              </span>{' '}
        ...
        <div className="flex flex-col bg-white dark:bg-black rounded-lg p-4 shadow-md h-1/2">
          <h1 className="text-lg font-mont-bd">
            {t('customer-service.visit-details.participants')}
          </h1>
      ...
      <div className="flex flex-col bg-white dark:bg-black rounded-lg p-4 shadow-md h-full w-1/2">
        <h1 className="text-lg font-mont-bd">
          {t('customer-service.visit-details.orders')}
        </h1>
        <div className="flex flex-col gap-4 mt-4 px-4">
          {visitDetails.orders.length > 0 ? (
            visitDetails.orders.map((order: any) => (
              <div key={order.orderId} className="flex flex-col border-b pb-2">
                <p className="font-mont-bd">
                  {t('customer-service.visit-details.order-id')}:{' '}
                  {order.orderId}
                </p>
                <p>
                  <span className="font-mont-bd">
                ...
                <p>
                  <span className="font-mont-bd">
                    {t('customer-service.visit-details.note')}:
                  </span>{' '}
                  {order.note || t('customer-service.visit-details.no-notes')}
                </p>
                <p>
                  <span className="font-mont-bd">
                    {t('customer-service.visit-details.cost')}:
                  </span>{' '}
                  {order.cost}
                </p>
                <p>
                  <span className="font-mont-bd">
                    {t('customer-service.visit-details.status')}:
                  </span>{' '}
                  {order.status}
                </p>
                {order.assignedEmployee && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <p>
                      <span className="font-mont-bd">
                        {t('customer-service.visit-details.employee')}:
                      </span>{' '}

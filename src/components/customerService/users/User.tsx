...
const User: React.FC = () => {
  ...
  const handleBanUser = async (values: { days: number; hours: number; minutes: number }) => {
    try {
      ...
      const parts = []
      if (values.days > 0) parts.push(`${values.days} day${values.days > 1 ? 's' : ''}`)
      if (values.hours > 0) parts.push(`${values.hours} hour${values.hours > 1 ? 's' : ''}`)
      if (values.minutes > 0) parts.push(`${values.minutes} minute${values.minutes > 1 ? 's' : ''}`)
  ...
  const handleUnbanUser = async () => {
    try {
      ...
      setUnbanMessage(`User ${userInfo?.firstName} ${userInfo?.lastName} was unbanned.`)
    } catch (error) {
  ...
  return (
    <div className="z-[0] flex h-full w-full items-center justify-center gap-5 bg-grey-1 p-4 dark:bg-grey-6">
      ...
      <div className="flex flex-col gap-3 self-start p-4 bg-white rounded-lg shadow-md dark:bg-black dark:text-white">
        <div className="mb-5 flex items-center gap-4 w-[200px]">
          {isLoading ? (
            ...
            userInfo && (
              <>
                <img src={getImage(userInfo.photo, DefaultImage)} alt="User Avatar" className="h-8 w-8 rounded-full" />
                <h1 className="font-mont-bd text-xl">{userInfo.firstName} {userInfo.lastName}</h1>
              </>
            )
          )}
        </div>
        <button
          ...
          className="border-[1px] rounded-md p-1 bg-white text-primary hover:bg-primary hover:text-white"
          onClick={() => navigate(`/customer-service/users/${userInfo?.userId}`)}
        >
      ...
      {/* Ban User Dialog */}
      <Dialog open={isDialogOpen} onClose={() => setIsDialogOpen(false)} title="Ban User">
  {banMessage ? (
    ...
    <Formik initialValues={{ days: 0, hours: 0, minutes: 0 }} onSubmit={(values) => handleBanUser(values)}>
      {({ values, isSubmitting }) => {
        const isBanDisabled = values.days === 0 && values.hours === 0 && values.minutes === 0
          ...
          <Form>
            <div className="flex flex-col justify-between min-h-[400px] min-w-[400px] px-6 py-4">
              <div className="flex flex-col gap-4">
                {/* D */}
                <div>
                  <label htmlFor="days" className="block text-sm font-medium mb-1">Days</label>
                  <Field
                    as="select"
                    name="days"
                    id="days"
...
{/* Unban User Dialog */}
<Dialog open={!!unbanMessage} onClose={() => setUnbanMessage(null)} title="Unban User">

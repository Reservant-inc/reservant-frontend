...
const ComplaintDetails: React.FC<ComplaintDetailsProps> = ({ report, onClose }) => {
  if (!report) {
    return (
  ...
  return (
    <div className="w-full h-full p-4 bg-white dark:bg-black rounded-lg relative">
      ...
      <div className="space-y-4">
        ...
        <p><strong>Submitted By:</strong> {report.createdBy ? `${report.createdBy.firstName} ${report.createdBy.lastName}` : 'Unknown'}</p>
        <p><strong>Reported User:</strong> {report.reportedUser ? `${report.reportedUser.firstName} ${report.reportedUser.lastName}` : 'Unknown'}</p>
      </div>

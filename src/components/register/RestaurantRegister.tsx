...
const RestaurantRegister = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://172.21.40.127:12038/my-restaurant-groups/`,
        );
        if (!response.ok) {
  ...
  const validationSchema = yup.object({
    ...
    tin: yup
      .string()
      .matches(/^[0-9]{11}$/, t("errors.restaurant-register.tin.matches"))
        ...
        {(formik) => (
          <Form>
            <div className="form-container">
              ...
              <Popup
                buttonText="Assign to a Group"
                modalTitle="Group Assignment"
              >
                <div></div>
              </Popup>

              <button
                id="RestaurantRegisterSubmitButton"
                type="submit"

...
const MyEventsModal: React.FC<MyEventsModalProps> = ({ open, onClose }) => {
  const [activeTab, setActiveTab] = useState<'created' | 'interested'>(
    'created'
  )
  ...
  const [showRemoveInterestModal, setShowRemoveInterestModal] =
    useState<boolean>(false)

  // CreateThread Modal
  const [showCreateThreadModal, setShowCreateThreadModal] =
    useState<boolean>(false)
  ...
  const inactiveButtonStyles =
    'w-[180px] dark:bg-grey-6 bg-grey-0 rounded-lg text-primary hover:text-primary-2 dark:text-secondary dark:hover:text-secondary-2 hover:bg-grey-1 dark:hover:bg-grey-6 transition'
  ...
  return (
    <Modal
      ...
      aria-describedby="my-events-description"
    >
      <>
              ...
              <div className="mt-6">
                {activeTab === 'created' ? (
                        ...
                        {!isEventPast(event.time) && (
                          <>
                            <button
                              className=""
                              onClick={() =>
                                handleCreateThreadClick(event.eventId)
                              }
                            >
                  ...
                  interestedEvents.map(event => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => handleRemoveInterestClick(event.eventId)}
                      >
                        <RemoveCircleIcon />
                      </button>
                    </div>
                  ...
                  <p className="dark:text-white">
                    No events you are interested in.
                  </p>
                )}
              </div>
            </>

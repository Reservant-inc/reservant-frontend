...
const Thread: React.FC<ThreadProps> = ({ thread }) => {
  ...
  const handleSendMessage = async (message: string) => {
    try {
      fetchPOST(`/threads/${thread.threadId}/messages`, JSON.stringify({ contents: message }))
    } catch (error) {
      if (error instanceof FetchError) {
        console.log(error.formatErrors())
      } else {
    ...
    <div className="h-[calc(100%-3rem)] w-full z-[0]">
      {
        isEditing ? (
          ...
          <div className="flex flex-col h-full w-full">
            <div id="scrollableDiv" className="h-full w-full overflow-y-auto scroll flex flex-col-reverse">
              <InfiniteScroll
                dataLength={messages.length}
                next={() => setPage((prevPage) => prevPage + 1)}
                hasMore={hasMore}
                loader={<CircularProgress />}
                scrollableTarget="scrollableDiv" // Make sure the ID matches the scrollable div
                className="px-2 hidescroll"
              >
                <div className="h-full w-full flex flex-col gap-4">
                  {
                    messages.map((message, index) => (
                      <div key={index} className={`flex items-center gap-2 w-full ${index % 2 === 0 ? 'self-start' : 'self-end flex-row-reverse'}`}>
                        <img className="h-6 w-6 rounded-full" src={DefaultUser} />
                        <h1 className={`text-sm rounded-lg break-words max-w-[70%] p-2 ${index % 2 === 0 ? 'bg-grey-1' : "bg-primary text-white"}`}>{message.contents}</h1>
                      </div>
                    ))
                  }
                </div>

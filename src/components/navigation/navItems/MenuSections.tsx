...
const Sections: React.FC<SectionProps> = () => {
    ...
    const isClickedRestaurants = location.pathname === `/${JSON.parse(Cookies.get('userInfo') as string).login}/restaurants`
    ...
    return(
        <div className="h-full flex gap-3 items-center">
                ...
                { isClickedHome && <span className="h-[3px] w-full bg-primary-2 dark:bg-secondary absolute bottom-0 transition transform scale-y-100"/> }
            </div>
            <div className="relative h-14">
                <button id="NavbarRestaurantsSectionButton" className={'h-12 mt-1 w-28 rounded-xl fill-grey-2 flex flex-col justify-between items-center relative' + (isClickedRestaurants ? ' fill-primary-2 dark:fill-secondary dark:hover:bg-none hover:bg-transpartent' : " dark:hover:bg-grey-4 hover:bg-grey-1")} onClick={() => navigate( `/${JSON.parse(Cookies.get('userInfo') as string).login}/restaurants`)}>   
                ...
                { isClickedRestaurants && <span className="h-[3px] w-full bg-primary-2 dark:bg-secondary absolute bottom-0 transition transform scale-y-100"/> }
            </div>
        </div>
    )
}

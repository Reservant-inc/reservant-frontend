...
const EventHistory: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"created" | "interested" | "participated">("created");
  ...
  const fetchEvents = async () => {
    try {
      ...
      const [createdResponse, interestedResponse, participatedResponse] = await Promise.all([
  ...
  const handleTabChange = (tab: "created" | "interested" | "participated") => {
    setActiveTab(tab);
    const eventsToShow = tab === "created" ? createdEvents :
                         tab === "interested" ? interestedEvents : participatedEvents;
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const eventsToFilter = activeTab === "created" ? createdEvents :
                           activeTab === "interested" ? interestedEvents : participatedEvents;
    setFilteredEvents(query.length >= 3 ? eventsToFilter.filter((event) => event.name.toLowerCase().includes(query)) : eventsToFilter);
  };
    ...
    <div className="flex flex-col bg-white rounded-lg w-full h-fit">
      {loading ? (
        ...
        {/* Nie wyświetlaj zakładek  jeżeli pathname zawiera /events */}
          {showTabs && <ProfileTabs activeTab={activeTab} onTabChange={handleTabChange} />}
          ...
          <div className="p-4 rounded-lg shadow-md">
            <div className="flex w-full items-center rounded-full border-[1px] border-grey-1 px-1 font-mont-md dark:border-grey-6 mb-4">
              <input type="text" placeholder="Search events" className="w-full placeholder:text-grey-2" onChange={handleSearchChange} />
              ...
              {filteredEvents.length === 0 ? (
                ...
                filteredEvents.map((event) => (
                  <EventDetails
                    ...
                    onDelete={() => { setShowDeleteDialog(true); setEventToDelete(event); }}
                    onManageParticipants={() => { setShowManageParticipantsDialog(true); fetchEventDetails(event.eventId); }}
                    onEdit={() => console.log("Edytuj")}
                    onLeave={() => { setShowLeaveDialog(true); setEventToDelete(event); }}
          ...
          {showDetailsDialog && (
            <Dialog open={showDetailsDialog} onClose={() => setShowDetailsDialog(false)} title="Szczegóły Wydarzenia">
              <div className="p-4">
                <p>Tu szczegóły eventu ewentualnie dla Działo się
                </p>
              </div>
            </Dialog>
          )}
          ...
          {showDeleteDialog && eventToDelete && (
            <Dialog open={showDeleteDialog} onClose={() => setShowDeleteDialog(false)} title="">
              <div className="p-4">
                <p className="mb-4">Czy na pewno chcesz {activeTab === "created" ? "usunąć wydarzenie" : "usunąć zainteresowanie"} {eventToDelete.name}?</p>
                <div className="flex justify-end gap-2">
                  <button className="bg-primary hover:bg-primary-2 text-white my-2 py-1 px-3 rounded transition hover:scale-105" onClick={handleDeleteEvent}>Tak</button>
                  <button className="bg-primary-2 hover:bg-primary-3 text-white my-2 py-1 px-3 rounded transition hover:scale-105" onClick={() => setShowDeleteDialog(false)}>Nie</button>
                </div>
              </div>
            </Dialog>
          )}
          ...
          {showLeaveDialog && eventToDelete && (
            <Dialog open={showLeaveDialog} onClose={() => setShowLeaveDialog(false)} title="">
              <div className="p-4">
                <p className="mb-4">Czy na pewno chcesz opuścić wydarzenie {eventToDelete.name}?</p>
                <div className="flex justify-end gap-2">
                  <button className="bg-primary hover:bg-primary-2 text-white my-2 py-1 px-3 rounded transition hover:scale-105" onClick={handleLeaveEvent}>Tak</button>
                  <button className="bg-primary-2 hover:bg-primary-3 text-white my-2 py-1 px-3 rounded transition hover:scale-105" onClick={() => setShowLeaveDialog(false)}>Nie</button>
                </div>
              </div>
            </Dialog>
          )}
          ...
          {showManageParticipantsDialog && (
            <Dialog open={showManageParticipantsDialog} onClose={() => setShowManageParticipantsDialog(false)} title="">
                      ...
                      {eventDetails && eventDetails.participants.length === 0 ? (
                        ...
                        eventDetails?.participants.map((participant) => (
                          <div key={participant.userId} className="flex items-center mb-3 gap-4">
                            <img src={getImage(participant.photo, UserDefault)} alt={`${participant.firstName} ${participant.lastName}`} className="h-10 w-10 rounded-full" />
                      ...
                      {interestedUsers.length === 0 ? (
                        ...
                        interestedUsers.map((user) => (
                          <div key={user.userId} className="flex items-center mb-3 gap-4">
                            <img src={getImage(user.photo, UserDefault)} alt={`${user.lastName}`} className="h-10 w-10 rounded-full" />
                            <span>{user.firstName} {user.lastName}</span>
                            <div className="flex ml-auto gap-1">
                              <button className="flex h-8 w-8 items-center justify-center rounded-md p-1 text-sm text-grey-2 hover:text-red" onClick={() => handleRejectUser(eventDetails!.eventId, user.userId)}>
                                <CloseSharpIcon className="h-5 w-5" />
                              </button>
                              <button className="flex h-8 w-8 items-center justify-center rounded-md p-1 text-sm text-grey-2 hover:text-green" onClick={() => handleAcceptUser(eventDetails!.eventId, user.userId)}>

...
const EventDetailsModal: React.FC<EventDetailsModalProps> = ({
  ...
  return (
    <>
        ...
        <div className="absolute top-1/4 left-1/2 transform -translate-x-1/2 -translate-y-1/4 w-96 bg-grey-0 dark:bg-grey-5 shadow-lg p-6 rounded-lg">
          ...
          {eventDetails && (
            <div className="dark:text-white">
              ...
              <p><strong>Created At:</strong> {formatDate(eventDetails.createdAt)}</p>
              <p><strong>Description:</strong> {eventDetails.description}</p>
              <p><strong>Event Time:</strong> {formatDate(eventDetails.time)}</p>
              <p><strong>Must Join Until:</strong> {formatDate(eventDetails.mustJoinUntil)}</p>
              <p><strong>Creator:</strong> {eventDetails.creatorFullName}</p>
              <p><strong>Restaurant:</strong> {eventDetails.restaurantName}</p>
              <p><strong>Participants:</strong> {calculateParticipants()} / {eventDetails.maxPeople}</p>
            </div>
          )}
      ...
      {/* MyEvents Modal */}
      <MyEventsModal open={showMyEventsModal} onClose={handleMyEventsModalClose} />
    </>
  );
};
